...
```{r}
ggplot() + geom_polygon(data = map_data_fortified, aes(fill=avg_total_fines, x=long, y=lat, group=group)) + geom_path(data = map_data_fortified, aes(x=long, y=lat, group=group), color='black', size=0.1) + coord_equal() + coord_proj("+proj=tmerc +lat_0=36.66666666666666 +lon_0=-88.33333333333333 +k=0.9999749999999999 +x_0=300000 +y_0=0 +ellps=GRS80 +datum=NAD83 +to_meter=0.3048006096012192 +no_defs") + scale_fill_viridis(option = "magma", direction = 1) + labs(title = "Amount owed from single ticketing \nincidents higher, on average, in city's \nsouth and west sides", caption='Source: ProPublica', x = "", y = "", fill="Total Fines per Incident") + theme(plot.title = element_text( size=16, face="bold", color='white'), panel.background = element_rect(fill = "#575757", colour = "#575757"),panel.grid.major=element_line(color='#575757'), panel.grid.minor=element_line(color='#575757'), plot.background = element_rect(fill = "#575757"), text=element_text(size=10, family="Yanone Kaffeesatz", color='white'), axis.text.y=element_text(color='#575757'),axis.ticks.length = unit(0, "cm"), axis.text.x=element_text(color='#575757'), legend.background = element_rect(fill='#575757')) 
```
...
```
Though much of the City's population density lies on the City's North Side, the number of city sticker tickets issued in the South far outweighs those issued in similarily populous, though denser areas of the North Side. To compare the number of issued tickets with the population size more plainly illustrates the higher issuance of tickets on the South and West Sides. Regardless of the various factors causing this phenomenon, that poorer communities are being hit with many more tickets should give policy makers pause about the effectiveness of the City's current methods of enforcing compliance among poorer residents.

```{r}
ggplot() + geom_polygon(data=sfshp_fortified, aes(fill = total_tickets, x=long, y=lat, group=group)) + geom_path(data = sfshp_fortified, aes(x=long, y=lat, group=group), color='gray', size=0.1) + coord_proj("+proj=tmerc +lat_0=36.66666666666666 +lon_0=-88.33333333333333 +k=0.9999749999999999 +x_0=300000 +y_0=0 +ellps=GRS80 +datum=NAD83 +to_meter=0.3048006096012192 +no_defs") + labs(title = "Scaling Zipcodes by Population Size Reveals \n   Disproportionate Sticker Ticketing in South and \nWest Side Communities (2017)", caption='Source: ProPublica and US Census', x = "", y = "", fill="Total Tickets") + theme(plot.title = element_text( size=14, face="bold", color='white', hjust=.5), panel.background = element_rect(fill = "#575757", colour = "#575757"),panel.grid.major=element_line(color='#575757'), panel.grid.minor=element_line(color='#575757'), plot.background = element_rect(fill = "#575757"), text=element_text(size=10, family="Yanone Kaffeesatz", color='white'), axis.text.y=element_text(color='#575757'),axis.ticks.length = unit(0, "cm"), axis.text.x=element_text(color='#575757'), legend.background = element_rect(fill='#575757')) + scale_fill_viridis(option = "cividis", direction = 1)
```
...
The following cartogram demonstrates the stark contrast of debts owed due for city sticker violations between lower income zipcodes and the wealthier zipcodes on the city's near and far North sides. That much of the City's economic activity is generated in the Chicago's downtown area makes the difference all the more striking. The relative difference between high and middle income zipcodes pales in comparison to the difference between high and low-income.

```{r}
ggplot() + geom_polygon(data=sfshp_fortified, aes(color = income_level, x=long, y=lat, group=group)) + geom_path(data = sfshp_fortified, aes(x=long, y=lat, group=group), color='gray', size=0.1) + labs(title = "Greater Sums of Total City Sticker Debt are Owed\n by Drivers in Low-Income Zipcodes (2017)", caption='Source: ProPublica', x = "", y = "", color="Income Level") + theme(plot.title = element_text( size=16, face="bold", color='white', hjust=.5), panel.background = element_rect(fill = "#575757", colour = "#575757"),panel.grid.major=element_line(color='#575757'), panel.grid.minor=element_line(color='#575757'), plot.background = element_rect(fill = "#575757"), text=element_text(size=10, family="Yanone Kaffeesatz", color='white'), axis.text.y=element_text(color='#575757'),axis.ticks.length = unit(0, "cm"), axis.text.x=element_text(color='#575757'), legend.background = element_rect(fill='#575757')) + scale_fill_viridis(option = "cividis", direction = 1) + coord_equal()
```
