...
```{r}
ggplot() + geom_polygon(data=sfshp_fortified, aes(color = income_level, x=long, y=lat, group=group)) + geom_path(data = sfshp_fortified, aes(x=long, y=lat, group=group), color='gray', size=0.1) + labs(title = "Totals of City Sticker Debt Owed\n by Drivers in Low-Income Zipcodes (2017)", caption='Source: ProPublica', x = "", y = "", fill="Total Tickets") + theme(plot.title = element_text( size=14, face="bold", color='white', hjust=.5), panel.background = element_rect(fill = "#575757", colour = "#575757"),panel.grid.major=element_line(color='#575757'), panel.grid.minor=element_line(color='#575757'), plot.background = element_rect(fill = "#575757"), text=element_text(size=10, family="Yanone Kaffeesatz", color='white'), axis.text.y=element_text(color='#575757'),axis.ticks.length = unit(0, "cm"), axis.text.x=element_text(color='#575757'), legend.background = element_rect(fill='#575757')) + scale_fill_viridis(option = "cividis", direction = 1) + coord_equal()
```
